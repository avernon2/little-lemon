...
const ConfirmedBooking: FC<HTMLAttributes<HTMLDivElement>> = (props): JSX.Element => {
  ...
  const [booking, setBooking] = useState<{ date: string; time: string; guests: string; occasion: string } | null>(null);
  ...
  return (
    <Container {...props}>
      ...
      <Button aria-label="Go back to the home page" onClick={() => navigate("/")}>
        Back to home
      </Button>
    </Container>
  );
